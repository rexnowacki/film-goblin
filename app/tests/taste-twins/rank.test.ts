...
describe("rankTasteTwins", () => {
  it("requires three evidence films and two shared facets, then orders by cosine", () => {
    ...
    const ranked = rankTasteTwins({ vector: viewer, evidenceFilmCount: 3 }, [
  ...
  it("does not call one-film viewers taste twins even when two facets overlap", () => {
    const candidate = {
      userId: "candidate",
      vector: { byTag: { folk: 5, bleak: 4 } },
      evidenceFilmCount: 3,
      watchlistOverlap: 0,
      secondDegree: false,
      sharedFilm: null,
    ...
    expect(rankTasteTwins(
      { vector: { byTag: { folk: 5, bleak: 4 } }, evidenceFilmCount: 1 },
      [candidate],
      facets,
      5,
    )).toEqual([]);
  ...
  it("labels cold-start fallbacks honestly and returns none without evidence", () => {
    ...
    expect(rankTasteTwins({ vector: { byTag: {} }, evidenceFilmCount: 0 }, [
  ...
  it("uses a stable user-id tie break and limit", () => {
    ...
    expect(rankTasteTwins({ vector: { byTag: {} }, evidenceFilmCount: 0 }, [{ ...base, userId: "b" }, { ...base, userId: "a" }], facets, 1)[0].userId).toBe("a");
  });
});
