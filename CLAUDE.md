...
**Last updated:** 2026-04-27 (end of session that shipped activity comments — 140-char inline threads, plus watch-note rendering, hover quick-add on Discovery, and watchlist exclusion from /films)

**Last shipped:** Activity comments (140-char inline threads). New `activity_comments` table (migration 0129) — surrogate UUID PK, `1..140` CHECK on body, two-disjunct DELETE policy (author OR activity owner). New `comment_on_activity` notification kind + SECURITY DEFINER trigger split across migrations 0130/0131. Server actions `addActivityComment` / `deleteActivityComment` (private/public split, optimistic-friendly returns). Read helper `getCommentSummariesForActivities`. New `CommentButton` (speech-bubble toggle) + `ActivityCommentThread` (inline list + 140-char composer with optimistic insert/delete, "Hide" + "Delete" pill affordances). Wired once into `ActivityFooter` so all 7 per-kind components inherit; group rows skip per spec decision #3. Both `/home` and profile pages attach comments. Notification rendering deep-links to `/home?activity=<id>` with auto-expand on arrival. Plus three follow-on micro-projects in the same session: watch-logged note rendering on the feed (migration 0132), hover quick-add (`+` → Watchlist or Library) on `/films`, and watchlist exclusion from Discovery (`getFilms` now filters owned + watchlisted). Spec `2026-04-27-activity-comments-design.md`, plan `2026-04-27-activity-comments.md`. Migrations 0129/0130/0131/0132 applied to prod. Live at https://film-goblin.vercel.app.

**Next up:** User asked at end of session to make **library-add an activity feed event** (analogous to `watchlist_added` / `watch_logged`). Not shipped this session — picks up next. Likely shape: new `activity_kind` enum value `library_added`, AFTER INSERT trigger on `library`, new `ActivityLibraryAdded` component + `EnrichedActivity` variant + `getEnrichedFeed` case + profile-page enrichment case + a `groupFeed` registration if bursts of adds should group like watchlist/watched do.
...
- **PR #30 (activity-comments spec + plan docs) still OPEN** — same situation as PR #21 was at session start. Decide whether to merge it as the canonical record of the design or close it (the implementation is the authoritative state now).
- **PR #21 (Covenfolk merge spec/plan)** was carried over from the prior session and merged today.
...
- Activity-comments deferred: threaded replies, comment editing, email notifications, comments on grouped feed rows, @-mentions / markdown, spam reporting, comment pagination beyond inline scroll.
- Watch-note staleness: `watched.note` edits do NOT propagate to the activity payload (snapshotted at INSERT, mirrors `recommendation_sent`). Add an UPDATE trigger if edit propagation matters.
- Discovery: `getWatchlistedFilmIds` helper landed in #39 and stayed when #40 added watchlist exclusion to `getFilms`. The `initialOnWatchlist` prop on `PosterQuickAdd` is now always `false` for any visible film on `/films` — kept for defensive correctness if the component is reused on a surface that doesn't filter watchlisted.
- Types regen: `app/lib/supabase/types.ts` was hand-edited this session (no Supabase CLI on this machine — see two-machine note). Both `activity_comments` and `comment_on_activity` are in the file.
...
User asked at end of 2026-04-27 to make **library-add an activity feed event** (analogous to `watchlist_added` / `watch_logged`). Picks up next session. Future micro-projects on deck per the deferred lists in earlier specs — coven-overlap signals on `/p/[handle]`, owned/review badges, year-in-review on `/watched`, friend-of-friend discovery, threaded comment replies, comment editing.
