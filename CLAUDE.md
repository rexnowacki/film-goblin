...
**Next up:** Nothing queued. **Invite gate is live.** **Cron resolved (2026-05-08):** all 5 crons had been silent for 3+ days because `CRON_SECRET` in production had a bad/empty value (set 3d before the symptom showed up — Vercel marks it "Sensitive" so the dashboard can't display it). Every cron tick was firing and silently 401-ing; not visible in runtime logs because Hobby retention is ~1 hour. Rotated the secret, redeployed, manual smoke against `/api/cron/refresh-prices` returned `{"ok":true,"digest":{films_refreshed:100,price_changes:3,alerts_fired:1}}`. Schedule still trimmed to 2 (refresh-prices + send-notifications) but that's just operational — the underlying issue was the secret. Pre-launch high-priority: real sender email domain via Resend (sandbox blocks digests reaching real users); list detail page `/lists/[id]` (currently dead-ends from feed). Adjacent quick wins: trailer surface on `/film/[id]` (mig 0150 + `fg-trailers` populate columns, no UI); comment editing + pagination (threaded replies already shipped via mig 0157/0158/0159); `/settings` handle validation (regex mismatch with onboarding).

**Open threads:**
- **`CRON_SECRET` rotation procedure.** Vercel marks the variable "Sensitive" — its value can't be viewed/copied from the dashboard after creation, only rotated. If crons go silent again with no visible logs, suspect the secret first. Local copy lives at `.cron-secret` (gitignored, repo root); the file is the canonical reference for manual smokes. Rotation flow: generate `openssl rand -base64 32` → save to `.cron-secret` → paste into Vercel dashboard Settings → Env → `CRON_SECRET` → Edit → Save (sensitive toggle on) → `npx vercel deploy --prod --yes` from repo root → smoke with `curl -H "Authorization: Bearer $(cat .cron-secret)" https://film-goblin.vercel.app/api/cron/refresh-prices` (expect `{"ok":true,"digest":{...}}`). Resolved 2026-05-08; previous secret had a bad/empty value, every cron tick was silently 401-ing, Hobby's 1h log retention masked it.
