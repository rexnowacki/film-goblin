...
**Next up:** Nothing queued. The FYP recommender + match-band UI shipped in #35→#37 (with the math expert's review patches). Mobile poster-tap action sheets shipped in #130–#133. Coven chip rework in #34. Tagging system v2 + editorial pass in #33 + #113. /home rewritten with infinite scroll + coven-scoped feed in #126–#129. Most queued items closed out this session.

Adjacent candidates still on the menu: tag the remaining 42 films via `/admin/films?untagged=1` (operational, unblocks better FYP signal); validate the math-review fixes in #37 with real user behavior over a few days; rating pills on poster grids using `films_with_stats.coven_rating_pct` (smaller scope); list detail page `/lists/[id]`; Your Ledger widget on /home (placeholder still); real sender email domain via Resend. Comment-surface deferred follow-ups still open: threaded replies, emoji quick-react strip, comment editing/pagination/@-mentions/markdown.
...
- **`algo.md` is at repo root, untracked.** Math expert's review reference. Worth committing for future re-reviews — flag it next session.
...
- **Calibration helper unwired but file preserved.** `app/lib/queries/fyp/calibration.ts` + its 23 specs still exist but no longer called from `getForYou`. Kept in case we revive a calibrated %-display later (with proper LOO at that point).
