...
**Next up:** Nothing queued. **Invite gate is live** — create admin batch codes via `/admin/invite-codes` before sharing the app. Adjacent quick wins: curate ~42 remaining untagged films via `/admin/films?untagged=1`; rating pills on poster grids (`films_with_stats.coven_rating_pct`); list detail page `/lists/[id]`; real sender email domain via Resend; trailer surface on `/film/[id]` (mig 0150 + `fg-trailers` populate columns, no UI yet); threaded comment replies.
