...
**Tier-zero hygiene (optional pre-C2 sweep):** fix `0119_films_with_stats_view.sql` to use `DROP VIEW IF EXISTS + CREATE VIEW` so pg-mem migration smoke passes; retrofit remaining `app/tests/actions/*.test.ts` files with `describe.skipIf` so the suite reports clean green-skipped instead of red-crashed when env is unset; align `/watchlist` hero with the post-B1 compressed shape that `/films` and `/library` already use. ~half a day of work; turns "tests pass" into a real signal before bigger features land.
