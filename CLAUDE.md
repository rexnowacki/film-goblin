...
**Next up:** Nothing queued. **Invite gate is live.** **Cron diagnosis (2026-05-08):** all 5 daily crons silently disabled by Hobby-plan 2-job cap — zero `/api/cron/*` invocations in 30+ days. Trimmed `app/vercel.json` to `refresh-prices` + `send-notifications` only; `send-rate-reminders`, `theater-alerts`, `check-itunes-availability` route code still ships but is no longer scheduled. Pre-launch high-priority: real sender email domain via Resend (sandbox blocks digests reaching real users); upgrade to Pro to restore the 3 dropped crons OR trigger them manually via curl on demand; list detail page `/lists/[id]` (currently dead-ends from feed). Adjacent quick wins: trailer surface on `/film/[id]` (mig 0150 + `fg-trailers` populate columns, no UI); comment editing + pagination (threaded replies already shipped via mig 0157/0158/0159); `/settings` handle validation (regex mismatch with onboarding).

**Open threads:**
- **Hobby-plan cron cap (2026-05-08).** Vercel Hobby allows only 2 cron jobs per project; configured 5, all were silently rejected (no log entries for any cron in 30+ days). Trimmed `app/vercel.json` to `refresh-prices` + `send-notifications`. To run a dropped cron manually: `curl -H "Authorization: Bearer $CRON_SECRET" https://film-goblin.vercel.app/api/cron/<path>`. To restore all 5: upgrade the project to Pro and re-add the 3 entries to `vercel.json`. Affected routes still ship and work — only their schedule is gone: `send-rate-reminders` (daily 11:00), `theater-alerts` (Mon/Thu 14:00), `check-itunes-availability` (Mon 14:00).
