...
**Last updated:** 2026-04-29 (end of session that shipped /watchlist redesign, /onboarding redesign, identity-at-signup, plus a long string of mobile UX polish — touch-action fixes, /coven horizontal-overflow, next/image for posters)

**Last shipped:** Three sub-projects landed in one session.

1. **Watchlist redesign (sub-project 18, PRs #49/50/51):** /watchlist became a poster grid identical in shape to /library. New `PosterDropBadge` ("23% OFF" top-right, ≥ 10% threshold) and a caption-line "Apple TV · $9.99 →" link beneath {year} · {director} (the on-poster buy pill was tried in #49 and removed in #51 after the user reported it didn't read as clickable; price was folded into the caption link). New `WatchlistSortChips` replaced the native `<select>`. `WatchlistRow.tsx`, `WatchlistSortSelect.tsx`, and ~110 lines of `.watchlist-row*` CSS deleted. Spec `2026-04-29-watchlist-redesign-design.md`. No migration.

2. **Onboarding redesign (sub-project 19, PR #52):** the 723-line five-chapter ritual collapsed to a single-page form (handle, threshold slider, pick three films). Hero matches /library / /watchlist; chapter labels, progress bar, blood-pact certificate, signature flourish, abandon button all gone. `OnboardingPayload` narrows from 7 fields to 3 — genres + storefronts dropped (captured-but-not-persisted today; restore as part of whatever future feature first reads them, per Path B); follow-seeding moved to /coven Find People. Films query now server-rendered. Spec `2026-04-29-onboarding-redesign-design.md`. No migration.

3. **Identity at signup (sub-project 20, PR #59 + #60 fix):** moved handle + display_name onto the signup form so identity is set deliberately at account creation, not patched in via onboarding's update-of-an-existing-row dance. Migration `0136` updates the `on_auth_user` trigger to read `raw_user_meta_data->>'handle'` + `'display_name'` when present (regex-validated), falling back to email-derived auto-generation for OAuth. `signUp` action validates both fields, pre-checks handle uniqueness via `serviceRoleClient`, passes via `auth.signUp({ options: { data } })`. Signup form has Display Name + Handle inputs; handle auto-suggests from display_name and locks on manual edit. `OnboardingForm` pre-fills handle from `profile.handle` so email/password users see what they picked at signup; OAuth users see auto-generated, can edit. PR #60 is a follow-up that adds `raw_user_meta_data jsonb NOT NULL DEFAULT '{}'` to the test mocks (`db/tests/helpers/auth-mock.sql` + `pg-mem.ts`) which broke CI when the trigger updated. Spec `2026-04-29-identity-at-signup-design.md`.

Plus a stack of supporting fixes: (a) **first-sign-in routes to /onboarding** via new `profiles.onboarded_at` column (migration `0135`), with the auth callback simplified and middleware extended to gate every authenticated request on the new flag (PRs #53/54); (b) **iOS form-zoom round 2** bumping inline `fontSize: 14` overrides on four more inputs/selects (PR #46); (c) **bottom-nav tap responsiveness** (PR #47); (d) **`next/image` for film posters** with `*.mzstatic.com` allowlist + per-surface variant sizing for AVIF/WebP (PR #48); (e) **iOS 300ms tap-zoom** killed first on `.bottom-nav__item` (#47), then on inputs/buttons site-wide (#55), then on `body` to catch text elements (#56); (f) **/coven horizontal overflow** fix — user reported "zooming in" on empty state, screenshot revealed clipped text/buttons; added `body { overflow-x: hidden }` at ≤720px and `flex-wrap` on `SearchPersonRow` so wide `.btn` CTAs drop below the avatar+name on narrow viewports (PR #57); (g) **handle validation + URL encoding** — user "tooth tony" typed "teeth tony" with a space; UserMenu's avatar link didn't `encodeURIComponent` (lone outlier), 404'd. Added regex `/^[a-z0-9._]+$/` to OnboardingForm + `_completeOnboarding`, encoded the link, patched Tony's handle in prod (PR #58).

Migrations 0135 and 0136 applied to prod. Live at https://film-goblin.vercel.app.

**Next up:** No specific next-up handed off this session. The user mentioned in passing that `/settings` could also use the same handle regex (today only OnboardingForm + `_completeOnboarding` validate; the settings form does a bare string update). That's a small follow-up and the obvious next breadcrumb. Beyond that, any of the deferred sub-projects below could pick up.
...
- **`/settings` handle validation gap.** OnboardingForm + `_completeOnboarding` now enforce `/^[a-z0-9._]+$/` on handles, but the `/settings` form (`SettingsForm.tsx` + `updateProfile` action) still does a bare string update. A user editing their handle later could still set a malformed one. Small follow-up PR.
...
- **B2 deferred** (carried from prior session): coven-scoped signals on `/p/[handle]`, owned + review badges, most-watched sort on `/films`, badges on `/library`/`/home` marquee/`/watched` strip, `/film/[id]` stat block beyond single caption, compact unit display past 99+.
- **Watch-note staleness** (carried): `watched.note` edits do NOT propagate to the activity payload (snapshotted at INSERT). Add an UPDATE trigger if edit propagation matters.
...
- **Types regen** (carried, now expanded): `app/lib/supabase/types.ts` was hand-edited this session for `profiles.onboarded_at`. Future regen via Supabase CLI on the other machine will need to preserve that field unless 0135 has been re-applied locally too.
...
- **`feedback_ios_zoom_horizontal_overflow.md`** auto-memory was added — when a user reports "page is zooming in" on iOS without a tap, suspect horizontal overflow + scroll-x: 0 before reaching for `touch-action` / 16px-input fixes. Ask for a screenshot first.
...
No specific next-up handed off this session. The user mentioned in passing that `/settings` could use the same handle regex as OnboardingForm + `_completeOnboarding` (small follow-up). Beyond that, future micro-projects on deck per the deferred lists in earlier specs — coven-overlap signals on `/p/[handle]`, owned/review badges, year-in-review on `/watched`, friend-of-friend discovery, threaded comment replies, comment editing, live handle-availability check on signup form, persisting `alert_threshold_pct` on profiles so users can change all alerts at once via `/settings`.
