...
**Open threads:**
- **Living Pit is new — watch the generators for a week.** `jobs.priceFeedScan` and `jobs.dailyFeedEvents` in the maintenance cron output (or `cron_runs` rows `price-feed-scan` / `daily-feed-events`); events land in `feed_events`. Landing + `/home` render them through the composer. If system rows vanish: check dedup suppression vs. an empty `feed_events` day — anniversaries are the fallback and 253 films now have real release dates. Deferred v2: `now_free`/`left_free` (provider snapshot/diff), `death_day`, ritual events; owner-spec's per-user feeds explicitly out of scope. Recorded-but-accepted minors live in the PR #180 review trail (`.superpowers/sdd/progress.md` was the working ledger, untracked).
