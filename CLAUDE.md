...
**Next up:** Nothing queued. **Invite gate is live.** **Cron mystery (2026-05-08, unresolved):** zero `/api/cron/*` invocations in 30+ days across all 5 configured crons. Initial diagnosis (Hobby 2-job cap) was WRONG — per Jan 2026 changelog all plans support up to 100 crons per project. Real cause unknown — needs dashboard investigation: Vercel → film-goblin → Cron Jobs tab to see if jobs are listed, their last-run timestamps, and whether something is paused. Trimmed `app/vercel.json` to `refresh-prices` + `send-notifications` operationally regardless (less surface, can be reverted once diagnosed). Endpoint code is fine — curl returns 401 properly. Pre-launch high-priority: real sender email domain via Resend (sandbox blocks digests reaching real users); list detail page `/lists/[id]` (currently dead-ends from feed). Adjacent quick wins: trailer surface on `/film/[id]` (mig 0150 + `fg-trailers` populate columns, no UI); comment editing + pagination (threaded replies already shipped via mig 0157/0158/0159); `/settings` handle validation (regex mismatch with onboarding).

**Open threads:**
- **Cron firing mystery (2026-05-08, unresolved).** Zero `/api/cron/*` invocations in Vercel runtime logs over 30+ days for any of the 5 originally-configured crons (refresh-prices, send-notifications, send-rate-reminders, theater-alerts, check-itunes-availability). Endpoint code is fine — curl with no auth header returns 401 from `/api/cron/refresh-prices` correctly. Initial Hobby-cap theory ruled out per Vercel's Jan 2026 changelog (all plans support 100 crons per project). Operationally trimmed `app/vercel.json` to `refresh-prices` + `send-notifications` while diagnosing; the trim doesn't fix the symptom. Real next step: open the Vercel dashboard's Cron Jobs tab for this project and check whether the jobs are listed, when they last ran, and whether anything is in a paused/disabled state. Also worth checking Usage in case some Hobby quota has been exhausted. To run any cron manually: `curl -H "Authorization: Bearer $CRON_SECRET" https://film-goblin.vercel.app/api/cron/<path>`.
- **iTunes cron — manual only.** Schedule dropped due to Hobby cap (above). Run on demand: `curl -H "Authorization: Bearer $CRON_SECRET" https://film-goblin.vercel.app/api/cron/check-itunes-availability`. After running, review fuzzy-match candidates at `/admin/itunes-candidates`. Rejection cooldown is 14 days before the route retries that film.
...
- **Sub-project #25 deferred:** comment editing/pagination/@-mentions/markdown; emoji quick-react strip; `LikersBottomSheet` for comment likes. (Threaded replies + "View N replies" UI shipped via mig 0157/0158/0159 + `CommentSheet`/`CommentList`.)
...
- **Rate-reminder cron** (mig 0146 + `/api/cron/send-rate-reminders`) — schedule dropped 2026-05-08 due to Hobby cap. Route still works; trigger via `curl -H "Authorization: Bearer $CRON_SECRET" https://film-goblin.vercel.app/api/cron/send-rate-reminders`. Opt-out via `profiles.notify_rate_reminders`. Bell deep-links to `/watched?rate=<watched_id>`.
- **Theater-alerts cron** (`/api/cron/theater-alerts`) — scrapes local theaters for showings of films in users' watchlist/grimoire and alerts them about big-screen runs. Schedule (Mon/Thu 14:00 UTC) dropped 2026-05-08 due to Hobby cap. Trigger via `curl -H "Authorization: Bearer $CRON_SECRET" https://film-goblin.vercel.app/api/cron/theater-alerts`. Has internal lock (`acquireCronLock`) so concurrent invocations no-op safely.
