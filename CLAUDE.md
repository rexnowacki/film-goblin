...
**Next up:** **FYP recommender (sub-project B of tagging)** is the natural follow-up to #33 — affinity scoring per user from `watched.recommended` × `film_tags`, candidate ranker, `/for-you` route, `/tags/<name>` listing pages, onboarding lane-picker. The hidden tail (positions 5+) shipped in #33 is what feeds it. Operational follow-up before FYP: curate tags on the catalog via the new editor at `/admin/films?untagged=1`. Adjacent quick wins still open: rating pills on poster grids using `films_with_stats.coven_rating_pct`; mobile poster-tap action sheet on `/films`. Real projects offered: Your Ledger widget on /home, real sender email domain via Resend, list detail page `/lists/[id]`. Roadmap entries: feed page parity + user-search, feed infinite scroll. Comment-surface deferred follow-ups still open: threaded replies, emoji quick-react strip, comment editing/pagination/@-mentions/markdown.
...
- **Sub-project #33 catalog still untagged.** Mig 0152 wiped v1's curation. Walk `/admin/films?untagged=1` and tag films per the staff guide (`/Users/christophernowacki/Downloads/filmgoblin-tagging-guide-v2.pdf`) before FYP can produce useful recommendations. Operational, not engineering.
- **Sub-project #33 tests not run locally.** RLS testcontainers + integration tests need Docker socket access, which isn't working on this machine (colima socket bind-mount issue). Will run on the other machine / in CI. The unit-style validation logic was reviewed inline; tests run against env-gated Supabase only.
...
- **Types regen.** `app/lib/supabase/types.ts` hand-edited for `profiles.email_added_at`, `profiles.email_*` (4 cols), `profiles.role`, `watched.recommended`, `films_with_stats.coven_rating_*`, `films.trailer_*` (6 cols), `films.horror_adjacent`, `tags.type` literal-union of six values, `film_tags.position` + `is_primary`. Future regen on the other machine will need to preserve all unless every migration has been re-applied locally.
