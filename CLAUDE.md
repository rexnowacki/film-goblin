...
**Last shipped (2026-05-07):** Four things. (1) **Film request / Summon (#39):** "Summon it →" on empty discover state → `FilmRequestSheet` multi-step flow. Fallback chain: iTunes → TMDB → manual. `film_requests` + `film_request_users` tables (mig 0170, 0171). Admin queue at `/admin/film-requests`. `film_request_fulfilled` notification + settings opt-out. (2) **Admin password reset:** `adminSendPasswordReset(userId)` — looks up email via service-role, dispatches Supabase reset to `/auth/reset`. `ResetPasswordButton` in Auth section on `/admin/users/[id]`. (3) **Feed tab fixes + watchlist search:** feed tabs (reviews/recs/lists) pass a `kinds` filter through server render + load-more so each tab gets its own page instead of 20 mixed rows. "From the Goblins" gated to `tab=all` only. Watchlist gains a search bar (title/director/year), client-side, no "summon" empty state. (4) **Admin last-activity:** `/admin/users/[id]` Auth section shows "Last activity" — most recent `activity.created_at`, fetched in parallel with other lookups.
...
**Next up:** Nothing queued. Adjacent quick wins: curate ~42 remaining untagged films via `/admin/films?untagged=1`; rating pills on poster grids (`films_with_stats.coven_rating_pct`); list detail page `/lists/[id]`; real sender email domain via Resend; trailer surface on `/film/[id]` (mig 0150 + `fg-trailers` populate columns, no UI yet); threaded comment replies.

**Open threads:**
- **~42 films still untagged** — 16 explicit non-horror cleanup, 26 needing editorial review. Walk `/admin/films?untagged=1`.
