...
**Last shipped (2026-07-03):** **FYP poster quick-add** (`feature/fyp-poster-quick-add`). The Browse tab's `PosterQuickAdd` affordance (desktop hover + menu; mobile ⋯ bottom sheet: log a watch / watchlist / grimoire / recommend / share) is now on the For You tab too — shelf posters (`ShelfCarousel`) and the Daily Omen hero (`DailyOmenHero`). The "Not interested" ✕ moved to the top-left corner on both surfaces (quick-add owns top-right, matching Browse); `.poster-quick-add .match-pill { right: 44px }` keeps the band pill clear of the ⋯ button. `ForYouSection` in `app/app/films/page.tsx` fetches the viewer's `watchlists`/`library` film-id sets + username in parallel for honest initial ✓ state (the FYP pool doesn't exclude saved films, so this matters). `PosterQuickAdd` itself unchanged; no migrations, no new actions. **Manual UI pass still owed** — checklist in plan Task 3 (`docs/superpowers/plans/2026-07-03-fyp-poster-quick-add.md`): mobile pill/⋯ spacing, hero ✕ at its new top-left spot, ✓-disabled state on saved films. Spec: `docs/superpowers/specs/2026-07-03-fyp-poster-quick-add-design.md`.
