...
## Amendment (2026-07-07): stride cap replaces pure-recency merge for Rule 2

Owner report: after Living Pit v2 landed, a Monday cron burst (several different-type system events emitted seconds apart) surfaced as a solid wall of goblin rows above all real user activity on both `/home` and the anon landing — expected under pure recency merge (rule 2 only barred *same-type* stacking) whenever a burst is fresher than the last stale user action, which is the common case at current volume. `composeFeed` (`app/lib/feed-events/compose.ts`) now enforces a stronger guard: whenever the previous placed row was ANY system event and a user item remains anywhere in the pool (even one older than the queued system events), the next row placed MUST be that user item. The soft same-type-avoidance guard from v1 is preserved as a secondary check once the hard guard doesn't apply. Once every user item has been placed, remaining system events surface in plain recency order — stacking at that point is unavoidable (nothing left to spread them across) and accepted. Ratio cap, priority selection, and date-seeded determinism are unchanged; this only reorders which already-chosen items are adjacent. Covered by two new `compose.test.ts` cases: a same-burst clump interleaving with stale user activity, and the fully-maxed 2:1 cap case where one adjacent system pair is mathematically unavoidable and must land only after every user item is placed.
